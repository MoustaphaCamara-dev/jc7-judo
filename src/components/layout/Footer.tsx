...
export default function Footer() {
  return (
    <footer className="w-full bg-jc7-white">
      <div className="mx-auto w-full px-4 py-12 sm:px-6 lg:px-8">
        <nav className="flex flex-wrap justify-center space-x-6 md:space-x-12" aria-label="Footer">
          {navigation.main.map((item) => (
            <Link
              key={item.name}
              to={item.href}
              className="text-sm leading-6 text-gray-600 hover:text-jc7-red"
            >
              {item.name}
            </Link>
          ))}
        ...
        <p className="mt-8 text-center text-xs leading-5 text-gray-500">
          &copy; {new Date().getFullYear()} JC7 Judo. Tous droits réservés.
        </p>
      </div>
    </footer>
  );
